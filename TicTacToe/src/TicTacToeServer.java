 ...
 *
 * Client -> Server Server -> Client ---------------- ---------------- MOVE
 * <n> (0 <= n <= 8) WELCOME <char> (char in {X, O}) QUIT VALID_MOVE
...
class Game {
	...
	/**
	 * A board has nine squares. Each square is either unowned or it is owned by
	 * a player. So we use a simple array of player references. If null, the
	 ...
	 * the move must be the current player and the square in which she is trying
	 * to move must not already be occupied. If the move is legal the game state
	 * is updated (the square is set and the next player becomes current) and
	 * the other player is notified of the move so it can update its client.
	 */
